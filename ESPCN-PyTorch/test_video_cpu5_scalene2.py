...
def main():
    ...
    # set up reading pipe
    command = [FFMPEG_BIN,
               '-vcodec', 'h264_cuvid', # un-comment to decode with GPU
    #           '-vcodec', 'h264', # un-comment to decode with CPU
               '-i', video_name,
               '-f', 'image2pipe',
               '-pix_fmt', 'bgr24',
               '-vcodec', 'rawvideo', '-an', '-']
    ...
    video_name_no_ext = os.path.basename(video_name)
    video_name_no_ext = os.path.splitext(video_name_no_ext)[0]
    output_video_file = f"espcn_{args.scale_factor}x_{video_name_no_ext}.h264"
    ...
    command = [FFMPEG_BIN,
            ...
            '-vcodec', 'h264_nvenc', # un-comment to encode with GPU
    #        '-vcodec', 'libx264', # un-comment to encode with CPU
            output_video_file]
    ...
    while True:
        ...
        if len(raw_frame) != (size[0]*size[1]*3):
            print('Error reading frame!!!')  # Break the loop in case of an error
            break


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Real-Time Single Image and Video Super-Resolution Using "
                                                 "an Efficient Sub-Pixel Convolutional Neural Network (CPU and GPU utilization).")
