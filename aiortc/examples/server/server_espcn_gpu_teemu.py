...
class VideoTransformTrack(MediaStreamTrack):
    ...
    async def recv(self):
        ...
        def SuperResolution():
            ...
            e1 = cv2.getTickCount()
            # Pre-processing
            img = frame.to_image().convert("YCbCr")
            y, cb, cr = img.split()
            ...
            cb_img = interpolate(cb_img, scale_factor=sf, mode='bicubic')
            cr_img = interpolate(cr_img, scale_factor=sf, mode='bicubic')
            cb_img *= 255.0
            cr_img *= 255.0
            ...
            print ("----------------------->SR Frame Size: ", sr_frame.size)

            # rebuild a VideoFrame, preserving timing information
            new_frame = VideoFrame.from_image(sr_frame)
